...
namespace SF.Feature.Handlebars
{
    public static class HandlebarManager
    {
        ...
        private static object getItemAsObject(Item item, int currentLevel, int maxLevel)
        {
            ...
            //Add some easy to consume Methods
            myDynamicItem.Add("IsPageEditorEditing", Sitecore.Context.PageMode.IsExperienceEditorEditing ? true : false);
            myDynamicItem.Add("ItemUrl", item.GetItemUrl());
            myDynamicItem.Add("ItemId", item.ID.ToString());

            //This is needed to get fields that have null value to show.
            item.Fields.ReadAll();

            foreach (Sitecore.Data.Fields.Field field in item.Fields)
            {
                string fieldValue = item.Fields[field.ID].Value.ToString();
                string propName = field.Name.Replace(" ", string.Empty);
                ...
                if (!field.IsStandardTemplateField() && !string.IsNullOrWhiteSpace(propName) && !myDynamicItem.ContainsKey(propName))
                {
                    ...
                    if (Sitecore.Data.Fields.FieldTypeManager.GetField(field) is Sitecore.Data.Fields.MultilistField)
                    {
                        ...
                        List<object> children = new List<object>();
                        foreach (var childItem in multiField.GetItems())
                        {
                            if (currentLevel < maxLevel)
                            {
                                children.Add(getItemAsObject(childItem, currentLevel, maxLevel));
                            }
                        }
                        myDynamicItem.Add(propName, children);
                        myDynamicItem.Add(propName + "_Count", children.Count);

                    }
                    else if (item.Fields[field.ID].Type.ToLower() == "droplink" ||
                        ...
                        item.Fields[field.ID].Type.ToLower() == "grouped droplink")
                    {
                        if (currentLevel < maxLevel)
                        {
                            ...
                            if (Guid.TryParse(item.Fields[field.ID].Value, out referenceID))
                            {
                                ...
                                var referenceObj = getItemAsObject(referencedItem, currentLevel, maxLevel);
                                myDynamicItem.Add(propName, referenceObj);
                            }
                    ...
                    else
                    {
                        if (Sitecore.Data.Fields.FieldTypeManager.GetField(field) is Sitecore.Data.Fields.LinkField)
                        {
                            ...
                            myDynamicItem.Add(propName + "_Url", linkUrl);
                            myDynamicItem.Add(propName + "_Text", linkField.Text);
                        }
                        ...
                        if (Sitecore.Data.Fields.FieldTypeManager.GetField(field) is Sitecore.Data.Fields.ImageField)
                        {
                            ...
                            if (imgField.MediaItem != null)
                            {
                                ...
                                myDynamicItem.Add(propName + "_Src", imgUrl);
                                myDynamicItem.Add(propName + "_Alt", altText);
                            }
                        }
                            ...
                            item.Fields[field.ID].Type.ToLower() == "datetime")
                        {
                            ...
                            {
                                myDynamicItem.Add(propName + "_ShortDate", dateField.DateTime.ToShortDateString());
                                myDynamicItem.Add(propName + "_LongDate", dateField.DateTime.ToLongDateString());
                            }
                        }

                        //Somehow children was getting in there.
                        if (propName != "Children")
                        {

                            string renderedValue = Sitecore.Web.UI.WebControls.FieldRenderer.Render(field.Item, field.Name);
                            myDynamicItem.Add(propName, renderedValue);

                            //Add Raw Value to context for use by helpers.
                            myDynamicItem.Add(propName + "_Value", field.Value);
                        }
                    }

                    if (item.HasChildren  && !myDynamicItem.ContainsKey("Children"))
                    {
                        List<object> children = new List<object>();
                        ...
                        myDynamicItem.Add("Children", children);
                        myDynamicItem.Add("Children_Count", children.Count);
                    }
                }
            }
